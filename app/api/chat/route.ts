...
import { userMemory, userPreferences } from "@/lib/db/schema";
...
export async function POST(req: Request) {
  ...
  const [chatTemplate, memoryRow, nativeLang, prefsRow] = await Promise.all([
    ...
    db
      .select({ currentTopic: userPreferences.currentTopic })
  ...
  const current_topic = prefsRow?.currentTopic || target_language || "not set";

    ...
    if (SpeechRecognition) {
      try {
        ...
        recognition.onresult = (event) => {
          ...
          if (!transcriptText) {
            setError("Speech recognition did not return any text.");
            return;
          }
          setTranscript(transcriptText);
          const expectedWords = normalize(exercise.sentence);
          const transcribedWords = normalize(transcriptText);
          ...
          onResultRef.current(correct, transcriptText);
        };

        recognition.onerror = (event) => {
          const err = event.error || "Speech recognition error";
          setError(`Could not transcribe audio: ${err}`);
        };
        ...
        // Fall through to MediaRecorder + server STT path
      }
    }
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          const message = e instanceof Error ? e.message : "Could not transcribe audio. Please try again.";
          setError(message);
        } finally {

 ...
 * icon: "📘"
 * color: "#4CAF50"
 * topic: "MedAT Biology"        # Universal topic (required)
 * targetLanguage: "de"          # Optional: only for language units
 * sourceLanguage: "en"          # Optional: only for language units
 * level: "B1"
...
export function parseUnitMarkdown(raw: string): ParsedUnit {
  ...
  return {
    ...
    topic: fm.topic ?? fm.targetLanguage ?? null, // fallback to targetLanguage for backward compat
